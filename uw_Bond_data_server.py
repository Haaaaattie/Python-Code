...
def GetBondData(month, day, starttime):  # compile high level data from each Bond Event into a dataframe
    ...
    timeoff = 7
    d = datetime(2019, month, day, starttime+timeoff)
...
if (p==1):
    ...
    #plotTemperature(243, LVFiles, LCFiles, BondDF)
    #plotTemperature(244, LVFiles, LCFiles, BondDF)
    #plotTemperature(245, LVFiles, LCFiles, BondDF)
    #plotTemperature(246, LVFiles, LCFiles, BondDF)
    #plotTemperature(247, LVFiles, LCFiles, BondDF)
    #plotTemperature(248, LVFiles, LCFiles, BondDF)
    plotTemperature(431, LVFiles, LCFiles, BondDF)

#%%
cols = BondDF.columns.tolist()
order = [0,8,69]

df1 = BondDF.pop('BondType') # remove column b and store it in df1
df2 = BondDF.pop('StartTime') # remove column x and store it in df2
df3 = BondDF.pop('BondSpeed')
df4 = BondDF.pop('BondTime')
df5 = BondDF.pop('HotPlateTemp')
df6 = BondDF.pop('BondHeadTemp')
df7 = BondDF.pop('NumberWafers')
df8 = BondDF.pop('Offsets')
df9 = BondDF.pop('NorthBuffer')
df10 = BondDF.pop('CenterBuffer')
df11 = BondDF.pop('SouthBuffer')
df12 = BondDF.pop('EastPress')
df13 = BondDF.pop('WestPress')
df14 = BondDF.pop('PressPos')
df15 = BondDF.pop('BondheadSerial')
df16 = BondDF.pop('BondEventNotes')
df17 = BondDF.pop('_id')
df18 = BondDF.pop('Error')
df19 = BondDF.pop('EndTime')


BondDF.insert(1,'BondType',df1)
BondDF.insert(2,'StartTime',df2)
BondDF.insert(3,'BondSpeed',df3)
BondDF.insert(4,'BondTime',df4)
BondDF.insert(5,'HotPlateTemp',df5)
BondDF.insert(6,'BondHeadTemp',df6)
BondDF.insert(7,'NumberWafers',df7)
BondDF.insert(8,'Offsets',df8)
BondDF.insert(9,'NorthBuffer',df9)
BondDF.insert(10,'CenterBuffer',df10)
BondDF.insert(11,'SouthBuffer',df11)
BondDF.insert(12,'EastPress',df12)
BondDF.insert(13,'WestPress',df13)
BondDF.insert(14,'PressPos',df14)
BondDF.insert(15,'BondheadSerial',df15)
BondDF.insert(16,'BondEventNotes',df16)
BondDF.insert(17,'_id',df17)
BondDF.insert(18,'Error',df18)
BondDF.insert(3,'EndTime',df19)
...
runs_means = compare_runs(509, 510, LVFiles, BondDF, LCFiles)
runs_means = compare_runs(510, 511, LVFiles, BondDF, LCFiles)
runs_means = compare_runs(511, 512, LVFiles, BondDF, LCFiles)
runs_means = compare_runs(512, 513, LVFiles, BondDF, LCFiles)
runs_means = compare_runs(513, 514, LVFiles, BondDF, LCFiles)
runs_means = compare_runs(514, 515, LVFiles, BondDF, LCFiles)
runs_means = compare_runs(515, 516, LVFiles, BondDF, LCFiles)
runs_means = compare_runs(516, 517, LVFiles, BondDF, LCFiles)

#%%
runs_means = compare_runs(311, 312, LVFiles, BondDF, LCFiles)
runs_means = compare_runs(276, 394, LVFiles, BondDF, LCFiles)
runs_means = compare_runs(510, 511, LVFiles, BondDF, LCFiles)


#%%
runs_means["East_Offsets"] = (runs_means["East1"]-runs_means["East2"])/100
runs_means["West_Offsets"] = (runs_means["West1"]-runs_means["West2"])/100
runs_means.to_csv("OffsetsProfile",index=False)
#def update_offsets(runs_means, Bond_profile)
...
def PlotAll(num):
    re = 2
    while re < num:
        plotLVandLCdata(re, re+1, "HotPlatePosition", LVFiles, LCFiles, num, BondDF)
        re += 2
    return
...
print("LVDT Pos (\u03bcm)")
#options for exporting to PDF: FPDF, ReportLab, pydf2, 
